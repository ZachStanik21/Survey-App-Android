        ...
        */
        ...
        if let geoNotificationString = notification.object as? String {
            ...
            if let parsedData = try? JSONSerialization.jsonObject(with: data, options: []) as! [String: Any] {
                ...
                log("didReceiveTransition url: \(concatUrl)")
                
                let task = URLSession.shared.dataTask(with: remoteUrl!) { data, response, error in
                    guard error == nil else {
                        log("didReceiveTransition error: \(error!)")
                        return
                    }
                    
                    guard let data = data else {
                        log("didReceiveTransition data empty")
                        return
                    }
                    
                    let myjson = try! JSONSerialization.jsonObject(with: data, options: []) as! [String: Any]
                    let code_retour = myjson["code_retour"] as? Double
                    log("didReceiveTransition code return: \(code_retour!)")
                }
